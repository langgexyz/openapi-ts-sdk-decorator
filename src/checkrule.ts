...
import { ValidationResult, OpenAPINamingRule, APIOperation } from './rules';
import { getAPIMethodsMetadata, getRootUri, type APIMethodMetadata } from './decorators';
    ...
    methodsMetadata.forEach(methodMetadata => {
      ...
      if (enableTypeValidation && moduleContext) {
        try {
          ...
          if (methodHasRequest && !hasExpectedRequest) {
            // 查找相似的Request类型
            const requestTypes = Object.keys(moduleContext).filter(name => 
              name.endsWith('Request') && typeof moduleContext[name] === 'function'
            );
            
            if (requestTypes.length > 0) {
              // 使用通用匹配算法找到最相关的类型
              const methodOperation = extractMethodOperation(methodMetadata.name);
              const relatedRequest = requestTypes.find(typeName => 
                isTypeRelatedToMethod(typeName, methodOperation)
              );
          ...
          if (!hasExpectedResponse) {
            // 查找相似的Response类型
            const responseTypes = Object.keys(moduleContext).filter(name => 
              name.endsWith('Response') && typeof moduleContext[name] === 'function'
            );
            
            if (responseTypes.length > 0) {
              // 使用通用匹配算法找到最相关的类型
              const methodOperation = extractMethodOperation(methodMetadata.name);
              const relatedResponse = responseTypes.find(typeName => 
                isTypeRelatedToMethod(typeName, methodOperation)
              );
      ...
      if (requireDocumentation) {
        if (!methodMetadata.options?.summary && !methodMetadata.options?.description) {
          ...
          suggestions.push(
            `[${methodMetadata.name}] Add summary or description to @${methodMetadata.method.toUpperCase()} decorator`,
            `[${methodMetadata.name}] Example: @${methodMetadata.method.toUpperCase()}('${methodMetadata.path}', { summary: 'Description here' })`
          );
        }
      }
    });
 ...
 */
function methodHasRequestParameter(client: APIClient, methodName: string): boolean {
  const PARAM_METADATA_KEY = '__openapi_ts_sdk_decorator_paramMetadata';
  
  try {
    const paramMetadata = (Reflect as any).getMetadata(PARAM_METADATA_KEY, client, methodName) || {};
    
    // 检查是否有任何参数使用了@Request()装饰器
    return Object.values(paramMetadata).some((meta: any) => meta && meta.type === 'request');
  } catch (error) {
    return false;
  }
}
 ...
 */
function extractParametersFromMetadata(methodMetadata: APIMethodMetadata): APIOperation['parameters'] {
 ...
 */
function getRealTypesFromClient(client: APIClient, methodName: string, moduleContext?: Record<string, new (...args: any[]) => any>): { requestType: (new (...args: any[]) => any) | null; responseType: (new (...args: any[]) => any) | null } {
  const PARAM_METADATA_KEY = '__openapi_ts_sdk_decorator_paramMetadata';
  
  try {
    // 获取参数装饰器元数据和类型信息
    const paramMetadata = (Reflect as any).getMetadata(PARAM_METADATA_KEY, client, methodName) || {};
    const paramTypes = (Reflect as any).getMetadata("design:paramtypes", client, methodName) || [];
 ...
 * 从方法名中提取核心操作信息
 */
function extractMethodOperation(methodName: string): {
  action: string;
  resource: string;
 ...
 */
function isTypeRelatedToMethod(typeName: string, methodOperation: { action: string; resource: string; fullOperation: string }): boolean {
  ...
  // 找到共同的关键词
  const commonWords = typeWords.filter(word => 
    methodWords.includes(word) && word.length > 2 // 忽略长度小于3的词
  );

  ...
  try {
    ...
    methodsMetadata.forEach(methodMetadata => {
      ...
      if (enableTypeValidation) {
        try {
          // 尝试从方法的实际调用中获取类型信息
          const method = (client as any)[methodMetadata.name];
          if (method && typeof method === 'function') {
            ...
            try {
              ...
              if (error?.message?.includes('Type naming validation failed')) {
                ...
                if (requestMatch) actualRequestType = requestMatch[1];
                if (responseMatch) actualResponseType = responseMatch[1];
                ...
                if (actualRequestType && actualResponseType) {
                  const typeValidationResult = OpenAPINamingRule.validateTypeNames(
                    methodMetadata.name,
                    actualRequestType,
                    actualResponseType
                  );
                  ...
                  if (!typeValidationResult.isValid) {
                    typeValidationResult.errors.forEach(err => 
                      errors.push(`[${methodMetadata.name}] ${err}`)
                    );
                    typeValidationResult.suggestions.forEach(sug => 
                      suggestions.push(`[${methodMetadata.name}] ${sug}`)
                    );
                  }
                } else {
                  // 如果无法解析类型名，添加通用错误
 ...
 */
function constructMethodArguments(client: APIClient, methodName: string, mockRequest: any, mockResponseType: any): any[] {
  const PARAM_METADATA_KEY = '__openapi_ts_sdk_decorator_paramMetadata';
 ...
 */
function getActualTypesFromMetadata(client: APIClient, methodName: string, moduleContext?: Record<string, new (...args: any[]) => any>): { requestType: (new (...args: any[]) => any) | null; responseType: (new (...args: any[]) => any) | null } {
  const PARAM_METADATA_KEY = '__openapi_ts_sdk_decorator_paramMetadata';

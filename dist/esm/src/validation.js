...
import { registerDecorator, buildMessage, Validator } from 'class-validator';
/**
        ...
        return function (target, context) {
            ...
            registerDecorator({
                ...
                validator: {
                    ...
                    defaultMessage: buildMessage((eachPrefix) => `${eachPrefix}$property must be a valid ${validatorName}`, validationOptions)
                }
            });
            // 对于新装饰器语法，返回 void
 ...
 * 验证属性值是否为字符串类型
 */
export const IsString = (validationOptions) => createCompatibleValidator('isString', (value) => typeof value === 'string', [])(validationOptions);
/**
 * 兼容 TypeScript 5.x 的 @IsNumber 装饰器
 * 验证属性值是否为数字类型
 */
export const IsNumber = (validationOptions) => createCompatibleValidator('isNumber', (value) => typeof value === 'number' && !isNaN(value), [])(validationOptions);
/**
 * 兼容 TypeScript 5.x 的 @IsBoolean 装饰器
 * 验证属性值是否为布尔类型
 */
export const IsBoolean = (validationOptions) => createCompatibleValidator('isBoolean', (value) => typeof value === 'boolean', [])(validationOptions);
/**
 * 兼容 TypeScript 5.x 的 @IsOptional 装饰器
 * 标记属性为可选，当值为 undefined 或 null 时跳过验证
 */
export const IsOptional = (validationOptions) => createCompatibleValidator('isOptional', () => true, // IsOptional 应该总是返回 true，它只是一个标记装饰器
[])(validationOptions);
/**
 * 兼容 TypeScript 5.x 的 @IsEmail 装饰器
 * 验证属性值是否为有效的邮箱地址
 */
export const IsEmail = (validationOptions) => createCompatibleValidator('isEmail', (value) => {
 ...
 * 验证字符串属性的最小长度
 */
export const MinLength = (min, validationOptions) => createCompatibleValidator('minLength', (value, args) => {
    if (typeof value !== 'string')
        return false;
 ...
 * 验证字符串属性的最大长度
 */
export const MaxLength = (max, validationOptions) => createCompatibleValidator('maxLength', (value, args) => {
    if (typeof value !== 'string')
        return false;
 ...
 * 验证属性值是否为数组类型
 */
export const IsArray = (validationOptions) => createCompatibleValidator('isArray', (value) => Array.isArray(value), [])(validationOptions);
/**
 * 兼容 TypeScript 5.x 的 @Min 装饰器
 * 验证数字属性的最小值
 */
export const Min = (min, validationOptions) => createCompatibleValidator('min', (value, args) => {
 ...
 * 验证数字属性的最大值
 */
export const Max = (max, validationOptions) => createCompatibleValidator('max', (value, args) => {
    if (typeof value !== 'number')
        return false;
 ...
 */
export const Type = (typeFunction, options) => createCompatibleType(typeFunction, options);
/**
 * 兼容 TypeScript 5.x 的 @ValidateNested 装饰器
 * 验证嵌套对象
 */
export const ValidateNested = (validationOptions) => createCompatibleValidator('validateNested', async (value, args) => {
    if (value == null)
 ...
 */
export { validate, ValidationError, ValidatorConstraint } from 'class-validator';
/**
 * 重新导出 class-transformer 的核心功能
 */
export { plainToClass, classToPlain, instanceToPlain, Transform, Expose, Exclude } from 'class-transformer';

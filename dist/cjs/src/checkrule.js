...
function checkRule(client, options) {
    ...
    const suggestions = [];
    try {
        const methodsMetadata = (0, decorators_1.getAPIMethodsMetadata)(client);
        // 检查是否有装饰方法
        ...
        methodsMetadata.forEach(methodMetadata => {
            ...
            if (enableNamingValidation) {
                const namingResult = rules_1.OpenAPINamingRule.validateMethodName(methodMetadata.name, operation);
                    ...
                    if (method && typeof method === 'function') {
                        try {
                            ...
                            if (actualTypes.requestType && actualTypes.responseType) {
                                ...
                                const expectedRequestName = rules_1.OpenAPINamingRule.generateRequestTypeName(methodMetadata.name);
                                const expectedResponseName = rules_1.OpenAPINamingRule.generateResponseTypeName(methodMetadata.name);
                            ...
                            if (error?.message?.includes('Type naming validation failed')) {
                                ...
                                // 生成期望的类型名
                                const expectedRequestType = rules_1.OpenAPINamingRule.generateRequestTypeName(methodMetadata.name);
                                const expectedResponseType = rules_1.OpenAPINamingRule.generateResponseTypeName(methodMetadata.name);
                                // 使用 OpenAPINamingRule.validateTypeNames 进行详细验证
                                if (actualRequestType && actualResponseType) {
                                    const typeValidationResult = rules_1.OpenAPINamingRule.validateTypeNames(methodMetadata.name, actualRequestType, actualResponseType);
            ...
            if (enableParameterValidation) {
                const expectedSignature = rules_1.OpenAPINamingRule.generateParameterSignature(operation);
                suggestions.push(`[${methodMetadata.name}] Expected signature: ${methodMetadata.name}${expectedSignature}`);
                const pathParams = rules_1.OpenAPINamingRule.extractPathParameters(operation.path);
...
function extractParametersFromMetadata(methodMetadata) {
    ...
    // 主要从路径中提取path参数
    const pathParams = rules_1.OpenAPINamingRule.extractPathParameters(methodMetadata.path);
